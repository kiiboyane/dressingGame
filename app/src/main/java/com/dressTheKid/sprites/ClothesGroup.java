...
public abstract class ClothesGroup {
    ...
    public Boolean wear( Game game , Sprite bodyPart ,  Image  empty , Boolean wflag   ,  AndroidSound sound){
    ...
    public  Boolean weardependly(Game game , Sprite bodyPart , Image empty , ArrayList<Boolean> flag , Boolean wflag   ){
        int who = whoIsdragged() ;
        if(who==-1) return wflag;
        if(wflag) return wflag;
        Sprite sprite = group.get(who);
