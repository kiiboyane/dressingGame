...
public class Game2Screen1 extends Screen  {
    private final String TAG = "Game2Screen1: ";
    ...
    public Game2Screen1(Game game) {
        ...
        addSprite(pants3);
        //adding tops
        addSprite(tops.group.get(0));
        addSprite(tops.group.get(1));
        addSprite(tops.group.get(2));
        // adding leftshoes
        ...
        new Thread(new Runnable() {
            public void run() {
                Shuffle.getBackground(g , 6);
                Shuffle.getGender(g , true  , 2 );
                Shuffle.getPants(g ,3);
                Shuffle.getTops(g , 5 );
    ...
    public void render(float deltaTime) {
        ...
        if(rflag && lflag && pflag && tflag  && waitabit > 30  && secondScreenReady )
        {
            ...
            game.setScreen(new Game2Screen2(game));
        }
    }
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        for(int i=pants.group.size()-1;i>=0;i--) {
            if(pants.group.get(i).contain(x,y)){
                pflag = pants.wearlevel2(game , legs , empty , pflag , GameSound.success.get(1)  ,i ,2) ;
                if(!pflag) GameSound.failurelevel.play(5);
        ...
        for(int i=tops.group.size()-1;i>=0;i--) {
            if(tops.group.get(i).contain(x,y)){
                tflag = tops.wearlevel2(game , chest , empty , tflag,GameSound.success.get(0) ,  i,1 );
                if(!tflag) GameSound.failurelevel.play(5);
