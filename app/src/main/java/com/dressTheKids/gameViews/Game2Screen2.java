...
public class Game2Screen2 extends Screen {
    private final String TAG = "Game2Screen2: ";
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        for(int i=pants.group.size()-1;i>=0;i--) {
            if(pants.group.get(i).contain(x,y)){
                pflag = pants.wearlevel2(game , legs , empty , pflag , GameSound.success.get(1)  ,i ,0) ;
        ...
        for(int i=tops.group.size()-1;i>=0;i--) {
            if(tops.group.get(i).contain(x,y)){
                tflag = tops.wearlevel2(game , chest , empty , tflag,GameSound.success.get(0) ,  i,0 );
